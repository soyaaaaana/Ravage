...
## ライセンスの表記について
`opensource_license.js` を追加することで、このリポジトリのライセンスを簡単に表記することが可能です。追加の際は、`main.js` もアップデートしてください。
<br>
なお、`opensource_license.js` の追加は原則必須です。古いバージョンの `main.js` を使用していて、別の方法でライセンスを表記している場合にのみ、`opensource_license.js` の追加は不要です。
